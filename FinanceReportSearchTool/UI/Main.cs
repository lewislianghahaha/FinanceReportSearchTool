...
namespace FinanceReportSearchTool.UI
{
    public partial class Main : Form
    {
        ...
        /// <param name="e"></param>
        private void Tmrp1_Click(object sender, EventArgs e)
        {
            try
            {
                //跳转至‘查询页面’,并将返回DT赋值至_dtl
                searchFrm.StartPosition=FormStartPosition.CenterScreen;
                searchFrm.ShowDialog();

                //判断若返回的结果为空,即报异常;反之,将结果赋值给gvdtl内
                if (searchFrm.ResultTable == null) return;

                if (searchFrm.ResultTable.Rows.Count==0)throw new Exception("没有查询到记录,请重新进行查询.");
                else
                {
                    //接收查询条件信息,并返回至lbmessage变量进行输出
                    lbmessage.Text = searchFrm.Lbmessage;

                    //判断若gvdtl内有数据,即先清空内容,再赋值
                    if (gvdtl?.RowCount > 0)
                    {
        ...
        private void Tmexport_Click(object sender, EventArgs e)
        {
            try
            {
                //判断若gvdtl没有内容,即不能执行操作
                if(gvdtl.RowCount==0)throw new Exception("不能执行操作,原因:没有内容");
                ...
                //从gvdtl内获取记录并赋值至task变量
                taskLogic.Add = fileAdd;
